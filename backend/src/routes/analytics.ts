...
analyticsRouter.get("/dashboard", verifyAdminToken, async (req: Request, res: Response) => {
  try {
...
analyticsRouter.get("/business-units", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const BUS = ["GCL", "LSF", "CAP", "UFL", "CHI", "UAC-Restaurants", "UPDC"];
    
    const stats = await Promise.all(
      BUS.map(async (bu) => {
        const userCount = await User.countDocuments({ businessUnit: bu });
        const adminCount = await AdminUser.countDocuments({ businessUnit: bu });
        const conversationCount = await Conversation.countDocuments({ "userId": { $exists: true } });
...
analyticsRouter.get("/popular-policies", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const limit = parseInt(req.query.limit as string) || 10;
    
    // Get policies sorted by relevance (for now, just by creation date, in real app you'd track views)
    const policies = await Policy.find({}, { title: 1, category: 1, businessUnit: 1, createdAt: 1 })
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();
...
analyticsRouter.get("/chat-activity", verifyAdminToken, async (req: Request, res: Response) => {
  try {
...
analyticsRouter.get("/usage-by-bu", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const BUS = ["GCL", "LSF", "CAP", "UFL", "CHI", "UAC-Restaurants", "UPDC"];
...
// Reset user password
analyticsRouter.post("/reset-password", verifyAdminToken, async (req: Request, res: Response) => {
  try {
...
analyticsRouter.get("/business-units-list", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const buses = await BusinessUnit.find().select("name label").sort("name");
...
analyticsRouter.post("/business-units", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    ...
    const newBU = new BusinessUnit({ name, label });
    await newBU.save();
...
analyticsRouter.put("/business-units/:id", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, label } = req.body;
...
analyticsRouter.delete("/business-units/:id", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    // Check if any admins or users are assigned to this BU
    const adminCount = await AdminUser.countDocuments({ businessUnit: id });
    const userCount = await User.countDocuments({ businessUnit: id });
...
analyticsRouter.get("/email-domains", verifyAdminToken, async (req: Request, res: Response) => {
  try {
...
analyticsRouter.post("/email-domain", verifyAdminToken, async (req: Request, res: Response) => {
  try {
...
analyticsRouter.delete("/email-domain/:id", verifyAdminToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
